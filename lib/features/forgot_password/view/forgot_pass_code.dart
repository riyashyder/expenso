...
class ForgotPassCode extends StatefulWidget {
  ...
  const ForgotPassCode({super.key, required this.email, required this.otpValidityTime});
...
class _ForgotPassCodeState extends State<ForgotPassCode> {
  ...
  void initState() {
    ...
    ForgotPassCodeController.otpControllers = List.generate(6, (_) => TextEditingController());
  ...
  void resendOtp() async {
    ...
    try {
      ...
      if (result is Map && result['error'] == null) {
        // Display full response body in SnackBar
        showSnackBar(context, "${jsonEncode(result)}");

        var data = result['data'];
        if (data != null && data.containsKey('otp_expiry_time')) {
          int otpValidityTime = int.parse(data['otp_expiry_time']) * 60;

          final timerController = Provider.of<OtpTimerController>(context, listen: false);
          timerController.restartTimer(otpValidityTime);
        }
      } else {
        showSnackBar(context, "Failed to resend OTP: ${jsonEncode(result)}");
      }
    } catch (e) {
  ...
  void showSnackBar(BuildContext context, String message) {
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(message, maxLines: 5, overflow: TextOverflow.ellipsis)),
      );
    }
  }
  ...
  //   //     child: CircularProgressIndicator(color: AppThemeData.primaryBackground),
  //   //   ),
  //   // );
  //
  ...
  Widget build(BuildContext context) {
    ...
    return MultiProvider(
      providers: [
        ChangeNotifierProvider<OtpTimerController>(
          create: (_) => OtpTimerController(widget.otpValidityTime),
        ),
        ChangeNotifierProvider<ForgotPassCodeController>(
      ...
      child: Builder(
        builder: (context) {
          final controller = context.watch<ForgotPassCodeController>();
                                      ...
                                      child: KeyboardListener(
                                        ...
                                        onKeyEvent: (event) {
                                          if (event is KeyDownEvent && event.logicalKey == LogicalKeyboardKey.backspace) {
                                            if (ForgotPassCodeController.otpControllers[index].text.isEmpty && index > 0) {
                                        ...
                                        child: Focus(
                                          ...
                                          child: TextField(
                                            controller: ForgotPassCodeController.otpControllers[index],
                                      ...
                                      child: AppElevatedButton(
                                        ...
                                        onPressed: controller.isOtpComplete && !controller.isLoading
                                            ? () async {
                                                ...
                                                await controller.verifyOtp(context, widget.email);
                                              }
                                            : null,
                                      ),
                                    )
